...
condition_local_poly <- function(knots, refined_x, p, s = NULL){
  ...
  Part1 <- local_poly_cpp(knots1, refined_x = refined_x_part1, p = p)
  Part1 <- rotate(rotate(Part1))
  
  Part2 <- local_poly_cpp(knots2, refined_x = refined_x_part2, p = p)
  ...
  if(prior.type == "PC"){
    if(likelihood == "Gaussian"){
      ...
      if(is.null(knots)){
        ...
        X = cbind(X, Xf)
        ## Design matrix for the spline basis weights
        B = as(local_poly_cpp(knots = knots, refined_x = x, p = p),"dgTMatrix")
        tmbdat <- list(
          # Design matrix
          X = X,
          B = B,
  ...
  if(likelihood == "Gaussian"){
    ...
    if(is.null(knots)){
      ...
      ff <- TMB::MakeADFun(
        ...
        parameters = tmbparams,
        random = "W",
        DLL = "00_RW_Poisson_Smoothing_PC",
        silent = TRUE
      )
      # Hessian not implemented for RE models
      ff$he <- function(w) numDeriv::jacobian(ff$gr,w)
      ...
      dyn.load(dynlib("00_RW_Poisson_Smoothing_PC"))
      ff <- TMB::MakeADFun(
        data = tmbdat,
        parameters = tmbparams,
        random = "W",
...
extract_mean_interval_refined_overd <- function(quad, n_samp = 3000, level = 0.95, p = 2, x, refined_x, type = 'overall', Xf = NULL, include.over = FALSE){
  pos_samps <- sample_marginal(quad, n_samp)
  ...
  if(type == "overall"){
    if(include.over){
      ...
      M <- cbind(B, X)
      fitted_samps_PTR3 <- M %*% head(pos_samps$samps, -length(refined_x))
    }
...
extract_deriv_samples_OSpline_overd <- function(quad, n_samp = 3000, p = 2, x, refined_x, degree){
  ...
  ## Design matrix for the spline basis weights
  B = as(local_poly_cpp(x, refined_x = refined_x, p = (p-degree)),"dgTMatrix")
...
extract_samples_Ospline_refined <- function(quad, n_samp = 3000, p = 2, x, refined_x, type = 'overall'){
  ...
  X = rbind(global_poly(refined_x[1], p = p), D_proposed[,1:p])
  ## Design matrix for the spline basis weights
  B = as(local_poly_cpp(x, refined_x = refined_x, p = p),"dgTMatrix")
  result <- list()
  if(type == "overall"){
